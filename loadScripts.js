// Load all the blog post insertion scripts
// Author: Andrew Jarombek
// Date: 4/19/2018

// Language Statistics
load('./stats/Bash.js');
load('./stats/C#.js');
load('./stats/C.js');
load('./stats/CSS.js');
load('./stats/Groovy.js');
load('./stats/Haskell.js');
load('./stats/HCL.js');
load('./stats/HTML.js');
load('./stats/Java.js');
load('./stats/JavaScript.js');
load('./stats/JSON.js');
load('./stats/Markdown.js');
load('./stats/PHP.js');
load('./stats/PLSQL.js');
load('./stats/Python.js');
load('./stats/Sass.js');
load('./stats/SQL.js');
load('./stats/Swift.js');
load('./stats/TypeScript.js');
load('./stats/XML.js');
load('./stats/YAML.js');

// 2017 Articles
load('./posts/2017/11-06-neo4j-create.js');
load('./posts/2017/11-08-js-scope-hoisting.js');
load('./posts/2017/11-09-js-closure-modules.js');
load('./posts/2017/11-10-es6-modules-babel.js');
load('./posts/2017/11-11-js-this.js');
load('./posts/2017/11-12-neo4j-challenges.js');
load('./posts/2017/11-13-js-global-object.js');
load('./posts/2017/11-14-sorting-lists.js');
load('./posts/2017/11-15-js-strict-mode.js');
load('./posts/2017/11-20-js-arrow-function.js');
load('./posts/2017/11-21-js-promises.js');
load('./posts/2017/11-25-generators.js');
load('./posts/2017/11-26-js-async-function.js');

load('./posts/2017/12-07-native-getter-setter.js');
load('./posts/2017/12-12-regex-captures.js');
load('./posts/2017/12-15-mongodb-pt1.js');
load('./posts/2017/12-16-mongodb-pt2.js');
load('./posts/2017/12-23-mongodb-pt3.js');
load('./posts/2017/12-27-mongodb-pt4.js');
load('./posts/2017/12-30-nodejs-mongodb-api-prototype.js');


// 2018 Articles
load('./posts/2018/01-03-string-swift.js');
load('./posts/2018/01-06-angular-5-first-impressions.js');
load('./posts/2018/01-14-shadow-dom.js');
load('./posts/2018/01-15-java8-strategy-pattern.js');
load('./posts/2018/01-16-java-default-method.js');
load('./posts/2018/01-19-swift4-objc.js');
load('./posts/2018/01-24-rabbitmq.js');
load('./posts/2018/01-27-angular-5-routing.js');
load('./posts/2018/01-30-java8-optionals.js');

load('./posts/2018/02-07-java8-completable-future.js');
load('./posts/2018/02-08-java8-functional.js');
load('./posts/2018/02-12-oracle-start.js');

load('./posts/2018/03-02-oracle-queries.js');
load('./posts/2018/03-08-typescript.js');
load('./posts/2018/03-09-travisci.js');
load('./posts/2018/03-10-sass.js');
load('./posts/2018/03-11-jwt.js');
load('./posts/2018/03-17-mean-stack-prototype.js');
load('./posts/2018/03-18-oracle-adv-pt1.js');
load('./posts/2018/03-19-oracle-adv-pt2.js');

load('./posts/2018/04-30-react-dynamic-jsx-elements.js');

load('./posts/2018/05-13-generics-arrays-complexities-java.js');
load('./posts/2018/05-15-java-clone.js');
load('./posts/2018/05-20-java-generics-api.js');
load('./posts/2018/05-23-javascript-immutable.js');
load('./posts/2018/05-26-nodejs-command-line.js');
load('./posts/2018/05-31-react-seed.js');

load('./posts/2018/06-04-webpack-seed.js');
load('./posts/2018/06-09-js-prototypes.js');
load('./posts/2018/06-13-web-workers.js');
load('./posts/2018/06-18-css-grid.js');
load('./posts/2018/06-23-bit-field.js');

load('./posts/2018/07-01-java-reflection.js');
load('./posts/2018/07-02-groovy-basics-pt1.js');
load('./posts/2018/07-04-groovy-basics-pt2.js');
load('./posts/2018/07-11-groovy-regex.js');
load('./posts/2018/07-15-groovy-optional-typing.js');
load('./posts/2018/07-29-overloading.js');
load('./posts/2018/07-31-batch-scripting.js');

load('./posts/2018/08-05-graphql-pt1.js');
load('./posts/2018/08-09-graphql-pt2.js');
load('./posts/2018/08-16-groovy-closures.js');
load('./posts/2018/08-27-groovy-currying.js');

load('./posts/2018/09-03-terraform.js');
load('./posts/2018/09-07-aws-lambda-api-gateway.js');
load('./posts/2018/09-14-groovy-dynamic.js');
load('./posts/2018/09-20-intro-programming.js');
load('./posts/2018/09-21-jenkins.js');
load('./posts/2018/09-24-python-data-model.js');

load('./posts/2018/10-04-python-data-structures.js');
load('./posts/2018/10-06-haskell-pt1.js');
load('./posts/2018/10-09-unicode.js');
load('./posts/2018/10-20-haskell-pt2.js');

load('./posts/2018/11-06-haskell-pt3.js');
load('./posts/2018/11-07-react-d3-pt1.js');
load('./posts/2018/11-10-react-d3-pt2.js');
load('./posts/2018/11-17-haskell-pt4.js');
load('./posts/2018/11-24-angular-lifecycles.js');

load('./posts/2018/12-14-python-class-features.js');
load('./posts/2018/12-15-python-protocols-abcs.js');
load('./posts/2018/12-22-multiple-inheritance.js');

// 2019 Articles
load('./posts/2019/01-01-haskell-pt5.js');
load('./posts/2019/01-02-csharp-first-impressions.js');
load('./posts/2019/01-03-cpp-first-impressions.js');
load('./posts/2019/01-07-webassembly.js');
load('./posts/2019/01-19-react-lifecycles.js');
load('./posts/2019/01-24-css-grid-backwards-compatibility.js');

load('./posts/2019/02-03-variance-csharp-generics.js');
load('./posts/2019/02-22-bash-scripting.js');

load('./posts/2019/03-12-angular-onpush.js');
load('./posts/2019/03-15-powershell-scripting.js');

load('./posts/2019/04-01-docker-pt1.js');
load('./posts/2019/04-08-docker-pt2.js');
load('./posts/2019/04-28-docker-pt3.js');

load('./posts/2019/05-12-csharp-delegates.js');
load('./posts/2019/05-13-kubernetes-pt1.js');
load('./posts/2019/05-20-kubernetes-pt2.js');
load('./posts/2019/05-28-haskell-pt6.js');

load('./posts/2019/06-17-terraform-module.js');

load('./posts/2019/08-03-haskell-pt7.js');
load('./posts/2019/08-18-type-equality.js');
load('./posts/2019/08-24-flexbox.js');

load('./posts/2019/09-03-rds-backups-pt1.js');
load('./posts/2019/09-05-rds-backups-pt2.js');
load('./posts/2019/09-15-elasticsearch.js');
load('./posts/2019/09-17-linq.js');
load('./posts/2019/09-29-linq-sqlserver.js');